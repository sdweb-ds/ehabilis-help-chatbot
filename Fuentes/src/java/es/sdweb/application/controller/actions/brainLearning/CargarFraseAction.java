...
import es.sdweb.application.controller.actionforms.brainLearning.CargarFraseForm;
...
/**
 * Implementa la funcionalidad Cargar Frase (comando load).
 */
public class CargarFraseAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN CargarFraseAction.execute()");
		...
		CargarFraseForm cargarFraseForm = (CargarFraseForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (cargarFraseForm != null) && (cargarFraseForm.getTextoEntrada() != null) && (!cargarFraseForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(cargarFraseForm.getTextoEntrada()); //Leemos una entrada del usuario

				LParrafo par=engineNLP.addParrafo(userInput);

				String salida = "";
				salida += "Frase procesada.";
	            ...
	            cargarFraseForm.setTextoSalida(salida);

			} else {
				cargarFraseForm.setTextoSalida("[Error] No se ha especificado una oración a cargar.");
			}

		} else {
			cargarFraseForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaCargarFrase");
		Log.logRT(usrDTO.getUsr(), "END CargarFraseAction.execute()");
