...
import es.sdweb.application.controller.actionforms.memoryCorp.EnviarFraseChatBotForm;
...
/**
 * Implementa la funcionalidad Enviar frase al ChatBot (loadbot).
 */
public class EnviarFraseChatBotAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN EnviarFraseChatBotAction.execute()");
		...
		EnviarFraseChatBotForm enviarFraseChatBotForm = (EnviarFraseChatBotForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (enviarFraseChatBotForm != null) && (enviarFraseChatBotForm.getTextoEntrada() != null) && (!enviarFraseChatBotForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(enviarFraseChatBotForm.getTextoEntrada()); //Leemos una entrada del usuario

				String answer = dialogueEngine.getAnswer(userInput);

				enviarFraseChatBotForm.setTextoSalida(answer);

			} else {
				enviarFraseChatBotForm.setTextoSalida("[Error] No se ha especificado una oración a cargar.");
			}

		} else {
			enviarFraseChatBotForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaEnviarFraseChatBot");
		Log.logRT(usrDTO.getUsr(), "END EnviarFraseChatBotAction.execute()");
