...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerCualidadesForm;
...
/**
 * Implementa la funcionalidad Obtener cualidades (comando props).
 */
public class ObtenerCualidadesAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerCualidadesAction.execute()");
		...
		ObtenerCualidadesForm obtenerCualidadesForm = (ObtenerCualidadesForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerCualidadesForm != null) && (obtenerCualidadesForm.getTextoEntrada() != null) && (!obtenerCualidadesForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerCualidadesForm.getTextoEntrada()); //Leemos una entrada del usuario
				...
				salida += "Cualidades del sustantivo: "+cualidades;

				obtenerCualidadesForm.setTextoSalida(salida);

			} else {
				obtenerCualidadesForm.setTextoSalida("[Error] No se ha especificado un sustantivo a analizar.");
			}

		} else {
			obtenerCualidadesForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerCualidades");
		Log.logRT(usrDTO.getUsr(), "END ObtenerCualidadesAction.execute()");
