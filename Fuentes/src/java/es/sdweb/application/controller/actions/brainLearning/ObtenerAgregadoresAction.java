...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerAgregadoresForm;
...
/**
 * Implementa la funcionalidad Obtener agregadores (comando agre).
 */
public class ObtenerAgregadoresAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerAgregadoresAction.execute()");
		...
		ObtenerAgregadoresForm obtenerAgregadoresForm = (ObtenerAgregadoresForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerAgregadoresForm != null) && (obtenerAgregadoresForm.getTextoEntrada() != null) && (!obtenerAgregadoresForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerAgregadoresForm.getTextoEntrada()); //Leemos una entrada del usuario

				String salida = "";

				salida += "Agregadores de \""+userInput+"\": ";
				String arbolComp=learningEngine.printAgregadores(userInput);
				salida += arbolComp;


				obtenerAgregadoresForm.setTextoSalida(salida);

			} else {
				obtenerAgregadoresForm.setTextoSalida("[Error] No se ha especificado una palabra para obtener agregadores.");
			}

		} else {
			obtenerAgregadoresForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerAgregadores");
		Log.logRT(usrDTO.getUsr(), "END ObtenerAgregadoresAction.execute()");
