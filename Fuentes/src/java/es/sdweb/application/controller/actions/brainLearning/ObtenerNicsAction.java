...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerNicsForm;
...
/**
 * Implementa la funcionalidad Obtener nics (comando nic).
 */
public class ObtenerNicsAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerNicsAction.execute()");
		...
		ObtenerNicsForm obtenerNicsForm = (ObtenerNicsForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerNicsForm != null) && (obtenerNicsForm.getTextoEntrada() != null) && (!obtenerNicsForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerNicsForm.getTextoEntrada()); //Leemos una entrada del usuario

                nics = NandaDefinitions.getNicString(userInput);

             } else {
                nics = NandaDefinitions.getNicsString();
             }

			String salida = nics;
			obtenerNicsForm.setTextoSalida(salida);

		} else {
			obtenerNicsForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerNics");
		Log.logRT(usrDTO.getUsr(), "END ObtenerNicsAction.execute()");
