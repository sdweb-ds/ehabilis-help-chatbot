...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerSinonimosForm;
...
/**
 * Implementa la funcionalidad Obtener Sinónimos (comando sin).
 */
public class ObtenerSinonimosAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerSinonimosAction.execute()");
		...
		ObtenerSinonimosForm obtenerSinonimosForm = (ObtenerSinonimosForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerSinonimosForm != null) && (obtenerSinonimosForm.getTextoEntrada() != null) && (!obtenerSinonimosForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerSinonimosForm.getTextoEntrada()); //Leemos una entrada del usuario
				...
				if (w!=null){
					salida += "Sinónimos de \""+userInput+"\": "+StringUtil.toString(w.getSinonimosStr(),true);
			    }else{
			        salida += "La palabra \""+userInput+"\" no está en el Diccionario";
			    }

				obtenerSinonimosForm.setTextoSalida(salida);

			} else {
				obtenerSinonimosForm.setTextoSalida("[Error] No se ha especificado una palabra a analizar.");
			}

		} else {
			obtenerSinonimosForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerSinonimos");
		Log.logRT(usrDTO.getUsr(), "END ObtenerSinonimosAction.execute()");
