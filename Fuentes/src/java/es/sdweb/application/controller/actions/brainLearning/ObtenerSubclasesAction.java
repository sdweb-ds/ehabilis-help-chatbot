...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerSubclasesForm;
...
/**
 * Implementa la funcionalidad Obtener subclases (comando sub).
 */
public class ObtenerSubclasesAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerSubclasesAction.execute()");
		...
		ObtenerSubclasesForm obtenerSubclasesForm = (ObtenerSubclasesForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerSubclasesForm != null) && (obtenerSubclasesForm.getTextoEntrada() != null) && (!obtenerSubclasesForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerSubclasesForm.getTextoEntrada()); //Leemos una entrada del usuario

				String salida = "";

				salida += "Subclases de \""+userInput+"\": ";
				String arbolComp=learningEngine.printJerarquiaDescendente(userInput);
				salida += arbolComp;


				obtenerSubclasesForm.setTextoSalida(salida);

			} else {
				obtenerSubclasesForm.setTextoSalida("[Error] No se ha especificado una palabra para obtener subclases.");
			}

		} else {
			obtenerSubclasesForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerSubclases");
		Log.logRT(usrDTO.getUsr(), "END ObtenerSubclasesAction.execute()");
