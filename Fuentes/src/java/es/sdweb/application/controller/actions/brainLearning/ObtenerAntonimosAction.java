...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerAntonimosForm;
...
/**
 * Implementa la funcionalidad Obtener Antónimos (comando ant).
 */
public class ObtenerAntonimosAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerAntonimosAction.execute()");
		...
		ObtenerAntonimosForm obtenerAntonimosForm = (ObtenerAntonimosForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerAntonimosForm != null) && (obtenerAntonimosForm.getTextoEntrada() != null) && (!obtenerAntonimosForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerAntonimosForm.getTextoEntrada()); //Leemos una entrada del usuario
				...
				if (w!=null){
					salida += "Antonimos de \""+userInput+"\": "+StringUtil.toString(w.getAntonimosStr(),true);
			    }else{
			        salida += "La palabra \""+userInput+"\" no está en el Diccionario";
			    }

				obtenerAntonimosForm.setTextoSalida(salida);

			} else {
				obtenerAntonimosForm.setTextoSalida("[Error] No se ha especificado una palabra a analizar.");
			}

		} else {
			obtenerAntonimosForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerAntonimos");
		Log.logRT(usrDTO.getUsr(), "END ObtenerAntonimosAction.execute()");
