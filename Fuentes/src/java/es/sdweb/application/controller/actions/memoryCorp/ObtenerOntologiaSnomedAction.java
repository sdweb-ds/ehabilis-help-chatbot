...
import es.sdweb.application.controller.actionforms.memoryCorp.ObtenerOntologiaSnomedForm;
...
/**
 * Implementa la funcionalidad Ontología SNOMED-CT (comando snomed).
 */
public class ObtenerOntologiaSnomedAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerOntologiaSnomedAction.execute()");
		...
		ObtenerOntologiaSnomedForm obtenerOntologiaSnomedForm = (ObtenerOntologiaSnomedForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			obtenerOntologiaSnomedForm.setTextoSalida(arbolSnomedCT);

		} else {
			obtenerOntologiaSnomedForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerOntologiaSnomed");
		Log.logRT(usrDTO.getUsr(), "END ObtenerOntologiaSnomedAction.execute()");
