...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerNandasForm;
...
/**
 * Implementa la funcionalidad Obtener nandas (comando nanda).
 */
public class ObtenerNandasAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerNandasAction.execute()");
		...
		ObtenerNandasForm obtenerNandasForm = (ObtenerNandasForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerNandasForm != null) && (obtenerNandasForm.getTextoEntrada() != null) && (!obtenerNandasForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerNandasForm.getTextoEntrada()); //Leemos una entrada del usuario

                nandas = NandaDefinitions.getNandaString(userInput);

             } else {
                nandas = NandaDefinitions.getNandasString();
             }

			String salida = nandas;
			obtenerNandasForm.setTextoSalida(salida);

		} else {
			obtenerNandasForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerNandas");
		Log.logRT(usrDTO.getUsr(), "END ObtenerNandasAction.execute()");
