...
import es.sdweb.application.controller.actionforms.brainLearning.ObtenerSuperclasesForm;
...
/**
 * Implementa la funcionalidad Obtener superclases (comando sup).
 */
public class ObtenerSuperclasesAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Log.logRT(usrDTO.getUsr(), "BEGIN ObtenerSuperclasesAction.execute()");
		...
		ObtenerSuperclasesForm obtenerSuperclasesForm = (ObtenerSuperclasesForm) actionForm;
		...
		if ( (accion != null) && (accion.equals("procesar")) ) {
			...
			if ( (obtenerSuperclasesForm != null) && (obtenerSuperclasesForm.getTextoEntrada() != null) && (!obtenerSuperclasesForm.getTextoEntrada().isEmpty()) ) {

				String userInput = StringUtil.trim(obtenerSuperclasesForm.getTextoEntrada()); //Leemos una entrada del usuario

				String salida = "";

				salida += "Superclases de \""+userInput+"\": ";
				String arbolComp=learningEngine.printJerarquiaAscendente(userInput);
				salida += arbolComp;


				obtenerSuperclasesForm.setTextoSalida(salida);

			} else {
				obtenerSuperclasesForm.setTextoSalida("[Error] No se ha especificado una palabra para obtener superclases.");
			}

		} else {
			obtenerSuperclasesForm.setTextoSalida(null);
		}

		result = actionMapping.findForward("PantallaObtenerSuperclases");
		Log.logRT(usrDTO.getUsr(), "END ObtenerSuperclasesAction.execute()");
